...
single detector and so cannot see blue or yellow.
    ...
    .venv/bin/python multi_pick.py --list              # every colour, all objects
    .venv/bin/python multi_pick.py --locate blue       # locate only, no motion
    .venv/bin/python multi_pick.py --dry-run blue      # full plan, no motion
    .venv/bin/python multi_pick.py blue                # execute

Filtering is in three stages, each needed:
  ...
  3. frame persistence — the detectors drop and invent blobs between frames
"""
import asyncio
import sys
...
async def candidates(machine, cam, detector):
    ...
    caller sampling several frames.
    """
    ...
    for d in sized:
        ...
        if in_workspace(pif.pose):
            out.append((pif.pose, d.x_max - d.x_min, d.y_max - d.y_min))
    return out


async def locate_colour(machine, cam, colour, frames=8, min_hits=4):
    ...
    for _ in range(frames):
        for pose, _w, _h in await candidates(machine, cam, detector):
            seen.append(pose)
...
async def main(argv):
    ...
    async with await connect() as machine:
        ...
        target = await locate_colour(machine, cam, colour)
        if target is None:
            return

        # Find the drop box now, while the arm is still at top-pose and can
        # see the whole table. After the pick it is down at the object and the
        # box may be out of frame.
        drop = None
        if not no_drop:
